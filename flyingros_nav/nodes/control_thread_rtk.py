...
def laser_callback(data):
    ...
    msg.pose.position.z = data.pose.position.z
...
def sendPosition():
    ...
    while run:
        ...
        msg.header.stamp=rospy.Time.now()
        msg.header.seq=laser_position_count
        msg.pose.position.x = lasers_raw.lasers[0]
        msg.pose.position.y = lasers_raw.lasers[1]
        msg.pose.position.z = lasers_raw.lasers[0]+lasers_raw.lasers[1]
...
def init(): 
    ...
    task_sub        = rospy.Subscriber('web/task', Task, Task_Callback, queue_size=1)
